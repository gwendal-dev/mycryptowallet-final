...
# JSON.parse(response)['Data'].each do |key, value|
#   arr_symbol << key
#   arr_images << "https://www.cryptocompare.com#{value['ImageUrl']}"
# end
# i = 0
# j = 0

# while i < arr_symbol.size
#       url = "https://api.lunarcrush.com/v2?data=assets&key=dobdvvfchtpmfr5qq1nu&symbol=#{arr_symbol[i]}"
#       uri = URI(url)
#       response = Net::HTTP.get(uri)
...
#         marketcap = nil
#       end
#       if marketcap != nil && arr_symbol[i] != "" && arr_images[i] != ""
#           Coin.create!([{title: arr_symbol[i], price: 1, avatar: arr_images[i], marketcap: marketcap, background: arr_images[i]}])
#           puts "-----------------------------------"
#           puts "#{j + 1} coins create"
#           puts "-----------------------------------"
#           j += 1
#       else
#         puts "n°#{i + 1} marketcap nil"
#       end
#       # puts "n°#{i + 1} error"
#   i += 1
# end
...
coins = Coin.all
  ...
  response = Net::HTTP.get(uri)
  coins.each do |coin|
    coin.description = JSON.parse(response)['Data'][coin.title]['Description']
    coin.save
